...
__global__ void calc_force_gpu(vector_dist_type vd, NN_type NN, real_number sigma12, real_number sigma6, real_number r_cut2)
{
	...
	while (Np.isNext())
	{
		...
		//atomicAdd(&vd.template getProp<force>(q)[1], 1);
		//atomicAdd(&vd.template getProp<force>(q)[2], 1);

		//atomicAdd(&vd.template getProp<force>(q)[0], -f.get(0));
		//atomicAdd(&vd.template getProp<force>(q)[1], -f.get(1));
		//atomicAdd(&vd.template getProp<force>(q)[2], -f.get(2));
...
template<typename CellList> void calc_forces(vector_dist_gpu<3,real_number, aggregate<real_number[3],real_number[3],real_number> > & vd, CellList & NN, real_number sigma12, real_number sigma6, real_number r_cut2)
{
    ...
    std::cout << "Number of cells :" << NN.getNCells() << std::endl;
    std::cout << "Elements distribution across NN cells:" << std::endl;
	int NelementsSum = 0;
    for (int i = 0; i < NN.getNCells(); i++) {
        if (NN.getNelements(i) > 0) {
            std::cout << "[" << i << ": " << NN.getNelements(i) << "] ";
			NelementsSum += NN.getNelements(i);
        }
    }
	std::cout << std::endl;
	...
	for (int i = 0; i < NN.numPartInCellPrefixSum.size(); i++) {
		std::cout << "[" << i << ": " << NN.numPartInCellPrefixSum.template get<0>(i) << "] ";
	}
	std::cout << std::endl;
#endif
	auto unsortToSort = NN.getNonSortToSort();
	std::cout << "unsortedToSortedIndex :" << unsortToSort.size() << std::endl;
	for (int i = 0; i < unsortToSort.size(); i++) {
		std::cout << "[" << i << ": " << unsortToSort.template get<0>(i) << "] ";
	}
	std::cout << std::endl;

	auto sortToUnsort = NN.getSortToNonSort();
	std::cout << "sortedToUnortedIndex :" << sortToUnsort.size() << std::endl;
	for (int i = 0; i < sortToUnsort.size(); i++) {
		std::cout << "[" << i << ": " << sortToUnsort.template get<0>(i) << "] ";
	}
	std::cout << std::endl;
