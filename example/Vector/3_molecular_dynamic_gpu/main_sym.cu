...
__global__ void calc_force_gpu(vector_dist_type vd, NN_type NN, real_number sigma12, real_number sigma6, real_number r_cut2)
{
	...
	while (Np.isNext())
	{
		...
		atomicAdd(&vd.template getProp<force>(p)[0], f.get(0));
		atomicAdd(&vd.template getProp<force>(p)[1], f.get(1));
		atomicAdd(&vd.template getProp<force>(p)[2], f.get(2));
...
int main(int argc, char* argv[])
{
	...
	auto NN = vd.getCellListGPU(r_cut, CL_SYMMETRIC, 2);
